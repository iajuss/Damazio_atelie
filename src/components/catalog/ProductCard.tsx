...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return <article className="product-card">
    ...
    <div className="product-card__body"><p className="product-card__line">{product.lineSlug.replaceAll('-', ' ')}</p><h3>{product.name}</h3>{product.description ? <p>{product.description}</p> : null}<div className="badges" aria-label="Características da peça"><span>Sob encomenda</span><span>Personalizável</span></div><Button href={`/produtos/${product.slug}`} variant="secondary">Ver detalhes de {product.name}</Button></div>
  </article>;
}
