...
export function ProductDetails({ product }: ProductDetailsProps) {
  ...
  return <div className="product-details">
    ...
    {isUnavailable ? <section className="product-details__unavailable" aria-live="polite"><p role="status">Esta peça não está disponível para solicitação no momento.</p><p>Para conhecer alternativas e próximas criações, fale com a Damazio pelo Direct.</p><Button href={INSTAGRAM_PROFILE_URL} variant="secondary">Conversar pelo Direct</Button></section> : <section className="product-details__request" aria-labelledby="sob-encomenda"><h2 id="sob-encomenda">Feita sob encomenda</h2><p>Cada criação começa com uma conversa para alinhar referências, materiais, prazo e os detalhes que tornam a peça sua.</p><Button href={`/solicitar-orcamento/${product.slug}`}>Solicitar orçamento</Button></section>}
  </div>;
}
