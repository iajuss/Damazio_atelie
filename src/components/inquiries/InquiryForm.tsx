...
export function InquiryForm({ product, requestKind = 'product', fetcher = fetch }: InquiryFormProps) {
  ...
  return <form className="inquiry-form" noValidate onSubmit={submit}>
    ...
    <div className="inquiry-field"><label htmlFor="description">Conte um pouco mais sobre sua ideia *</label><textarea id="description" name="description" rows={5} required {...fieldProps('description')} />{errors.description ? <p id={inputErrorId('description')} className="inquiry-field__error">{errors.description}</p> : null}</div>
