...
alter table public.inquiry_email_notifications
  add column recipient_kind text not null default 'atelier'
    check (recipient_kind in ('atelier', 'customer'));

alter table public.inquiry_email_notifications
  drop constraint inquiry_email_notifications_inquiry_id_key,
  add constraint inquiry_email_notifications_inquiry_recipient_kind_key unique (inquiry_id, recipient_kind);

create function public.create_inquiry_with_contact_details(
  p_request_code text,
  p_product_id uuid,
  p_request_kind text,
  p_name text,
...
drop function public.claim_inquiry_email_notifications(integer, text);
